...
ruby format-codes.rb
git branch -vv
git checkout -b basic_132
git push --set-upstream origin basic_132
git push
...
cd build && ninja
cd build && ninja help
cd build && ninja run run_v2 run_v3


#ALL
# #include(v3.cmake)
cd - && rm -rf build/ bin-project bin-target/
rm -rf build/ bin-project bin-target/
cmake -GNinja -H. -Bbuild
cd build
ninja
ninja -t targets all | rg run_v4
ninja run_v4
ls ../bin-project/v4_main_132
rm ../bin-project/v4_main_132
# #ALL
ninja
ninja -t targets all | rg run_v4
ninja run_v4
